    ...
    <el-main class="baoBScoll">
      <el-row style="height: 100%;">
        <el-col :span="24" style="height: 100%;">
          ...
          <el-row ref="print" class="baoBTopH" id="printTest">
            <h3 v-if="printT" style="text-align: center;">盘库统计</h3>
            <el-col :span="24" class="resWith" v-if="iHide" style="height: 100%;">
              <div class="baoBMain">
                <div class="table-container">
                  <el-table ref="brandTable"
                            ...
                            show-summary
                            border>
                    <el-table-column label="名称" sortable align="left">
                    ...
                    <el-table-column  label="条码" align="center" >
                      <template slot-scope="scope" sortable>{{scope.row.barcode}}</template>
                    </el-table-column>
                    <el-table-column prop="outQuantity" sortable label="使用量" width="150" align="center">
                      <template slot-scope="scope" sortable>{{scope.row.outQuantity}}</template>
                    </el-table-column>
                    <el-table-column prop="remnantQuantity" sortable label="剩余量" width="150" align="center">
                      <template slot-scope="scope" sortable>{{scope.row.remnantQuantity}}</template>
                    </el-table-column>
                    <el-table-column  sortable label="统计日期" width="200" align="center">
                      <template slot-scope="scope" sortable>{{scope.row.statiDate}}</template>
                    </el-table-column>
            ...
            <el-col :span="24" class="resImg" style="height: 100%;" id="print-content" v-else-if="iHide==false">
              <stateChart :chartData="chartData" :ids="ids" :extend="extend"  :settings="chartSettings" :data-zoom="dataZoom" ></stateChart>
              <div ref="chartPrint" style="opacity: 0;"></div>
            </el-col>
          </el-row>
...
<script>
    ...
    import {stockPageList,stockChart} from '@/api/baoBiao'
    import {
        deviceList
    } from '@/api/material'
    ...
    export default {
        ...
        methods: {
            getList() {
                ...
                stockPageList(this.listQuery).then(response => {
                    if(response.code !=='1'){
                        this.$message({
            ...
            inventoryChart(val){
                ...
                stockChart(sendData).then(response=> {
                    if(response.code !=='1'){
                        mv.$message({
                            message: response.msg,
                            type: 'error',
            ...
            export2Excel(){
                require.ensure([], () => {
                    ...
                    const tHeader = ['名称','规格','条码','使用量','剩余量','统计日期'];
                    const filterVal = ['name', 'size','barcode', 'outQuantity', 'remnantQuantity','statiDate'];
