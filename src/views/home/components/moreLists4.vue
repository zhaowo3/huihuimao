    ...
    methods: {
      ...
      querySearch(searchtxt) {
        ...
        searchS(this.Qs.stringify(hou)).then(res => {
          ...
          let listTotal = res.data.expiration.total;
          if (listTotal > 0) {
            this.busy = true
          } else {
            this.busy = false
          ...
          //订单
          this.loading = false;
          mv.leftInfo.houseList = mv.leftInfo.houseList.concat(res.data.expiration.result);
          let DownloadTotal =
            (mv.leftInfo.searchHouse.pageNumber + 1) * mv.leftInfo.searchHouse.pageSize;
          let total = res.data.expiration.total;
