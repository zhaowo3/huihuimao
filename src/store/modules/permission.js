...
function hasPermission(menus, route) {
  if (route.name) {
    ...
    if (currMenu!=null) {
      //设置菜单的标题、图标和可见性
      if (currMenu.meta.title != null && currMenu.meta.title !== '') {
        route.meta.title = currMenu.meta.title;
      }
      if (currMenu.meta.icon != null && currMenu.meta.title !== '') {
        route.meta.icon = currMenu.meta.icon;
      }
    ...
    GenerateRoutes({ commit }, data) {
      return new Promise(resolve => {
        const accessedRouters = asyncRouterMap.filter(v => {
          //admin帐号直接返回所有菜单
          let menus=asyncRouterMap;
          // if(username==='admin') return true;
          // let menus=data.roles;
