<template>
  <el-row class="detailsScroll">
    ...
    <div class="templateSubject">
      ...
      <el-col :span="24" class="templateLeft">
...
<!--          </li>-->
    ...
    <div class="footButtonNum">
      <p class="infoNum"><span>已显示:{{houseNum}},</span><span>总数:{{houseNum}}</span></p>
      <el-button @click="matinButton(danViews.id)" type="primary" style="float: right;">确定</el-button>
    </div>
  </el-row>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //     this.normal=this.houseList;
            //     if(val===''||val==null){
            //         this.normal=JSON.parse(JSON.stringify(this.normal));
            //         return;
            //     }
            //     let list=this.normal.filter(item=>item.materialName.indexOf(val)>=0);
            //     this.normal=list;
            //
            //     // mv.houseList = [];
            //     // mv.querySearch(val);
            // },
            reset(){
                this.normal=JSON.parse(JSON.stringify(this.houseList))
            },
